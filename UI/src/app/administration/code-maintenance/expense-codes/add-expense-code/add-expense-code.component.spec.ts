...
export class AddExpenseCodeComponent extends BaseFormComponent implements OnInit {
    ...
    OnActionComplete(args) {
        if (args.action == "add") {
            ...
            this.expenseCode = new ExpenseCodeModel();
            ...
            if (this.expenseCode.UserThresholds.length == 0) {
                // let data = this.form.get('UserThresholds') as FormArray;
                // data.patchValue(CurrentData);
                // this.ThresholdsDetails.patchValue(data.value);
                this.expenseCode.UserThresholds.push(CurrentData);
            }
        }
        if (args.action == "edit") {
            let data = this.expenseCode.UserThresholds;
            for (let i = 0; i < data.length; i++) {
                if (data[i].ID == args.data.ID) {
                    data[i].ThresholdAmount = args.data.ThresholdAmount;
                }
            }
            console.log(data);
            // const index = data.indexOf(args.previousData);
            // data.splice(index, 1);
